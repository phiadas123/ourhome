      ...
      <main className="page">
        {!loaded ? (
          ...
          <SectionView
            ...
            onAdd={(urls) => urls.forEach((u) => addOne(u, room, section))}
            onEdit={setEditingId}
...
function SectionView({ room, section, items, currency, onAdd, onEdit, onToggle, onRemove, onEditSection }) {
  const t = totals(items);
  return (
    <>
      <div className="room-head">
      ...
      <PasteBar where={section} onAdd={onAdd} />
      {!items.length && <p className="empty-note">Copy a product link from any shop and paste it above. The photo, name and price appear in a few seconds. You can paste several links at once.</p>}
      <div className="grid">
        {items.map((it) => (
          <Card key={it.id} item={it} currency={currency} onEdit={() => onEdit(it.id)} onToggle={() => onToggle(it)} onRemove={() => onRemove(it)} />
        ))}
      </div>
    </>
  );
...
function PasteBar({ where, onAdd }) {
  ...
  function submit(text) {
    ...
    onAdd(urls);
    setValue("");
    setHint(urls.length > 1 ? `Adding ${urls.length} pieces…` : "");
  ...
  return (
    <form className="paste" onSubmit={(e) => { e.preventDefault(); submit(value); }}>
      <input
        ...
        onPaste={(e) => { const text = e.clipboardData.getData("text"); if (findUrls(text).length) { e.preventDefault(); submit(text); } }}
        placeholder="Paste a link here"
        inputMode="url" autoComplete="off" aria-label={`Paste a product link for ${where}`}
      />
...
function Card({ item, currency, onEdit, onToggle, onRemove }) {
